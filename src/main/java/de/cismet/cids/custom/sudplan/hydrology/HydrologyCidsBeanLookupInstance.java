 ...
 */
@ServiceProvider(
    service = CidsBeanLookupInstance.class,
    position = 1
)
public final class HydrologyCidsBeanLookupInstance implements CidsBeanLookupInstance {

    //~ Methods ----------------------------------------------------------------

    @Override
    public Collection getLookupInstances(final CidsBean cidsBean) {
        if (SMSUtils.TABLENAME_HYDROLOGY_WORKSPACE.equals(cidsBean.getMetaObject().getMetaClass().getTableName())) {
     ...
     * @version  $Revision$, $Date$
     */
    private static final class HydroWorkspaceMapVisualisationProvider implements MapVisualisationProvider {
        ...
        public Feature getFeature(final CidsBean bean) {
            ...
            final WFSQueryInfo queryInfo = new WFSQueryInfo() {
                    ...
                    private final transient ShowCatchmentAreaForPointAction scafpa =
                        new ShowCatchmentAreaForPointAction();
